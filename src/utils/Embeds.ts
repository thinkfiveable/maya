...
export const Embeds = {
  success: (message: string, title?: string) => new MessageEmbed()
  ...
  permission: (permissions: string[]) => new MessageEmbed()
      ...
      .setDescription(
        `This requires the following permission(s): ${permissions}}`
      )
      .setFooter(
  ...
  cooldown: (ms: number) => new MessageEmbed()
      ...
      .setDescription(
        `Wait \`${Math.floor(ms / 1000)} seconds\` before trying again.`
      )
}
