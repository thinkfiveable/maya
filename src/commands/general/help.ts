...
export default class HelpCommand extends Command {
      ...
      args: [
        {
          ...
          type: 'commandAlias',
          prompt: {
            retry: Embeds.error('I couldn\'t find a command with that name. What command would you like to search for?', 'No command found.'),
            ended: Embeds.error('Sorry. I still couldn\'t find a command with that name. Use the command without any arguments to get a list of available commands.'),
            retries: 1,
            optional: true
          }
        }
  ...
  async exec (message: Message, { command }: { command?: Command }) {
    ...
    if (command.description.usage) {
      embed.addField('Usage',
        `\`\`\`${prefix}${command.aliases[0]} ${command.description.usage}\`\`\``)
    }
  ...
  async allCommands (message: Message) {
    ...
    for (const category of this.client.commandHandler.categories.values()) {
      ...
      embed.addField(`${category.id.replace(/(\b\w)/gi, (lc) => lc.toUpperCase())}`,
