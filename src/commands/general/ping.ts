...
export default class PingCommand extends Command {
  ...
  async exec (message: any) {
    ...
    const latency = (pong.editedTimestamp || pong.createdTimestamp) - (message.editedTimestamp || message.createdTimestamp)

    const pingEmbed = new MessageEmbed()
      .setColor(Colors.BLANK)
      .setDescription(`🏓 **Gateway Ping:** ${latency.toString()}ms\n🔄 **API Ping:** ${Math.round(this.client.ws.ping).toString()}ms`)
